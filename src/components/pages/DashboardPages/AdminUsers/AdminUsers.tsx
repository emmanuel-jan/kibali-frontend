...
const AdminUsers = (props: any) => {
  ...
  const { data: allUsers } = useGetAllUsersQuery(1);
  ...
  for (let i = 0; i < allUsers?.length; i++) {
    data.push({
      ...
      phone: allUsers[i]?.phone_number,
      status: [`${allUsers[i]?.is_instructor}`],
      parent_status: [`${allUsers[i]?.is_parent}`],
      student_status: [`${allUsers[i]?.is_student}`],
      action: allUsers[i]?.id,
    });
  }
